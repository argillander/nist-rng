...
def _test_runner_mt(name, bits, battery, check_eligibility, results):
    print("Running " + name + " (pid: " + str(os.getpid()) + ") ")
    results.append(run_by_name_battery(name, bits, battery, check_eligibility))

def run_all_battery_mt(bits, battery, check_eligibility=True):
    m = Manager()
    results = m.list()
    processes = []
    tests_to_skip = ["linear_complexity", "serial", "approximate_entropy", "binary_matrix_rank"]
    for name in battery.keys():
        if name in tests_to_skip:
            ...
            p = Process(target=_test_runner_mt, args=(name, bits, battery, check_eligibility, results,))
            processes.append(p)
            p.start()
...
if __name__ == "__main__":
   ...
   #print(sequence)
    ...
    results = run_all_battery_mt(binary_sequence, eligible_battery, False)
